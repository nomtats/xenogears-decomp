...
3. **Evaluate & Iterate:** Run `make asmdiff FUNC=<FunctionName>` inside Docker to compile and diff in one step. This invokes `tools/asmdiff.py --build`, which rebuilds the object file and then shows a color-coded, instruction-by-instruction diff between the target assembly and your compiled output. The tool auto-discovers the target `.s` file and the generated dump, masks relocation fields, and reports a match score. If the diff does not match exactly, analyze the mismatched positions and rewrite the code. Repeat this loop autonomously. For GUI-based comparison, `objdiff` instances (tracking `objdiff.json`) will also auto-reload.
4. **Post-Match Reflection:** As soon as a perfect match is achieved (or a function is abandoned), execute the **Post-Match Reflection skill** at `ai_workflow/skills/post_match_reflection/SKILL.md`. This 6-step checklist covers Doxygen documentation, goto reduction, structured outcome logging, KB updates, TOC dashboard updates, and readability scanning. Follow it instead of the less specific steps that were previously here.
...
- **The Goto Fallback Heuristic:** Modern structured C (`if`, `while`, `for`) is always preferred. However, if the original PS1 assembly features (1) explicit True Branch polarity (`bgtz`, `beq` jumping into logic rather than jumping over it), (2) unconsolidated return paths (multiple `jr $ra` statements with custom delay slots rather than merged crossjumping), or (3) variable assignments packed firmly into early-exit delay slots that standard C hoists out, you MUST fall back to a flat `goto` state-machine to force `GCC 2.7.2` to natively adopt the assembly structure mapping.
- **The Scheduling Escalation Ladder:** When the target and generated assembly differ in instruction ordering, apply fixes in escalating order — never skip steps: (1) Clean structured C (`if/else`, no goto) — solves control flow and block layout. (2) `register var asm("$N")` pinning — solves register allocation. (3) One `__asm__("")` barrier — splits scheduling blocks to prevent unwanted hoisting. (4) Last resort: explicit `__asm__("instruction")` with register-pinned args for instructions GCC won't place correctly. Add a comment block showing the equivalent clean C. See KB patterns 018–020 for details and anti-patterns.
