...
- **New tool: `tools/asmdiff.py`** (2026-03-14): CLI diff tool for per-instruction comparison between target `.s` and compiled dump. Auto-discovers files by function name, handles MIPS relocations, color-coded output with ✓/≈/✗ markers and match scoring. Usage: `python3 tools/asmdiff.py <FuncName>` or `make asmdiff FUNC=<FuncName>`. This replaces manual grep/sed/diff workflows and serves as the foundation for the Tier 5 Diff Interpretation Engine.

### Immediate Next Steps

1. **Continue MoveImage iteration** — 25+ attempts completed. Achieved 45/49 match using inverted if-body pattern (KB pattern_example_017) + register asm + __asm__("") barrier. Remaining 4 mismatches are a load-scheduling rotation at positions 35-38 that resists C-level fixes. Use `make asmdiff FUNC=MoveImage` to check progress. If still stuck after a few more attempts, consider accepting as near-match or revert to INCLUDE_ASM.
