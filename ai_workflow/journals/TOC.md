...
- **MoveImage matched 100% (49/49)** (2026-03-14): After 25+ attempts, MoveImage byte-matched using: inverted if-body pattern (KB 017) for `bnez` without goto, register pinning (`$2`=dest, `$3`=pkg), one `__asm__("")` barrier, and two explicit inline asm instructions for delay slot scheduling control. The hardest function so far — yielded 4 new KB patterns (018–020, anti_pattern_003).
- **ClearOTag/ClearOTagR matched** (2026-03-15): Both perfectly byte-for-byte matched via PsyQ standard macros (`setlen`/`setaddr`) and `GpuPackage` vtable dispatch. Added `D_8005698C` as a readability candidate.
- **Libgpu Sprint 2 matched** (2026-03-15): 5 functions perfectly byte-for-byte matched via delay slot analysis and pointer aliases (`SetDrawMove`, `PutDrawEnv`, `DrawOTagEnv`, `SetGraphReverse`, `SetGraphDebug`).
- **Remaining libgpu INCLUDE_ASM stubs**: 8 functions still unmatched (PutDispEnv, SetDispMask, ResetGraph, func_8004463C, func_8004440C, func_800444B8, func_80043EAC, func_80043F18).
- **Workflow: Tier 1 is 3/4 complete** (2026-03-14): Post-Match Reflection skill and Structured Outcome Records both created and committed. Outcome log bootstrapped with 47 retroactive entries. AGENT.md updated with Scheduling Escalation Ladder directive. Only the **Pattern Decision Tree** remains in Tier 1.
- **Upgraded tool: `tools/asmdiff.py`** (2026-03-15): CLI diff tool upgraded with sequence alignment and heuristic mismatch analysis. Usage: `make asmdiff FUNC=<FuncName>` inside Docker. Handles relocations, aligns blocks bypassing insertion drift, provides inline yellow highlighting for register mismatches, and suggests fixes via `--analyze`. Foundation for Tier 5 Diff Interpretation Engine.
- **KB expanded to 21 patterns** (2026-03-14): Added inline asm scheduling control (018), barrier dosage anti-pattern (019), scheduling escalation ladder methodology (020), and comprehensive MoveImage failure catalog (anti_pattern_003). Pattern Decision Tree mismatch→fix examples enriched with delay slot, branch polarity, and signedness entries.

### Immediate Next Steps

1. **Continue libgpu decompilation** — 8 INCLUDE_ASM stubs remain. `SetDispMask` may require `func_80047178` header info. `PutDispEnv` is large but mechanical. Use `make asmdiff FUNC=X` for iteration.
...
3. **Build the Pattern Decision Tree** (`ai_workflow/skills/pattern_classifier/SKILL.md`) — the last Tier 1 item. Now has richer mismatch→fix examples from MoveImage experience.
4. After Tier 1 is closed, evaluate whether to push into Tier 2 or do a larger decompilation sprint.
...
| 2026-03-14 | MoveImage Matched 100% (25+ attempts) | Resolved via inverted if-body pattern (bnez without goto), register pinning ($2/$3), barrier, and explicit inline asm for delay slot control. Hardest function to date. Created asmdiff.py tool. 4 new KB patterns + failure catalog. | [2026-03-14_2359_moveimage_matched.md](./2026-03-14_2359_moveimage_matched.md) |
| 2026-03-15 | ClearOTag Pair Match | Matched ClearOTag and ClearOTagR by implementing the raw setlen/setaddr loop and vtable dispatch via GpuPackage. Added D_8005698C as variable rename candidate. | [2026-03-15_0015_clearotag_pair_match.md](./2026-03-15_0015_clearotag_pair_match.md) |
| 2026-03-15 | Libgpu 5-batch match | Byte matched SetDrawMove, PutDrawEnv, DrawOTagEnv, SetGraphReverse, SetGraphDebug. Decoded GCC's global variables pointer reusing by manually defining local pointers to trick GCC. | [2026-03-15_0045_libgpu_batch_match.md](./2026-03-15_0045_libgpu_batch_match.md) |
| 2026-03-15 | Aligned Diff Upgrade | Upgraded `asmdiff.py` to use sequence alignment (`difflib.SequenceMatcher`). This prevents block insertions from destroying subsequent alignment. Added analytical heuristics and inline mnemonic diff highlighting. PutDispEnv match percentage jumped from 30% to 70%. | [2026-03-15_1534_asmdiff_aligned_diff.md](./2026-03-15_1534_asmdiff_aligned_diff.md) |
